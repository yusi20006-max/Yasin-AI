...
class EncryptionEngine:
    ...
    The caller supplies a high-entropy key generated by :meth:`generate_key`.
    For compatibility with the existing string key API, the key is represented as
    64 hexadecimal characters. A random PBKDF2 salt derives the AES-256 key,
    while a fresh 96-bit nonce is generated for every encryption operation.
    """
    ...
    def encrypt(cls, plaintext: str, key: str) -> str:
        ...
        The serialized format is ``salt || nonce || ciphertext+tag``. AES-GCM
        authenticates both the ciphertext and its associated metadata.
        """
