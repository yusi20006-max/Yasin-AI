...
def test_healthy_import_is_persisted_and_redacted(tmp_path: Path):
    ...
    status, response = bridge.import_credential({"provider": "openai", "credential": secret, "model": "gpt-test", "label": "primary"})
    assert status == 200
    assert response["imported"] is True
    assert secret not in str(response)
    assert registry.get_credential(response["credential"]["id"]) == secret
...
def test_duplicate_import_is_idempotent(tmp_path: Path):
    ...
    payload = {"provider": "openai", "credential": "credential-value-2"}
...
def test_unhealthy_credential_is_not_persisted(tmp_path: Path):
    ...
    status, response = bridge.import_credential({"provider": "openai", "credential": "credential-value-3"})
    assert status == 422
...
def test_import_rejects_validation_error_even_when_authenticated(tmp_path: Path):
    ...
    status, _ = bridge.import_credential({"provider": "openai", "credential": "credential-value-4"})
    assert status == 422
    assert registry.list_public() == []
...
def test_secret_never_appears_in_public_record(tmp_path: Path):
    registry = CredentialRegistry(tmp_path / "credentials.json")
    record, _ = registry.import_credential(provider="openai", credential="credential-value-5", metadata={"source": "token-manager"})
